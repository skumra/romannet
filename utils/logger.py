...
class Logger:
    ...
    def __init__(self, logging_directory, args=None):
        ...
        if logging_directory:
            ...
            self.base_directory = os.path.join(os.path.abspath('logs'), timestamp_value.strftime('%Y-%m-%d.%H:%M:%S'))
        ...
        if os.path.exists(self.base_directory):
            self.logging_directory_exists = True
        else:
            self.logging_directory_exists = False
        ...
        logging.basicConfig(
            level=logging.INFO,
            filename="{0}/{1}.log".format(self.base_directory, 'log'),
        ...
        if self.logging_directory_exists:
            logging.info('Pre-loaded data logging session: %s' % self.base_directory)
        else:
            logging.info('Created data logging session: %s' % self.base_directory)
    ...
    def save_model(self, iteration, model):
        torch.save(model.cpu().state_dict(), os.path.join(self.models_directory, 'snapshot-%06d.pth' % iteration))

    def save_backup_model(self, model):
        torch.save(model.state_dict(), os.path.join(self.models_directory, 'snapshot-backup.pth'))
    ...
    def save_visualizations(self, iteration, affordance_vis, name):
        ...
        cv2.imwrite(os.path.join(self.visualizations_directory, '%s.png' % name), affordance_vis)
