...
class GenerativeResnet4(nn.Module):
    ...
    def __init__(self, input_channels=4, output_channels=1, channel_size=32, dropout=False, prob=0.1):
        ...
        self.output = nn.Conv2d(in_channels=channel_size, out_channels=output_channels, kernel_size=2)

        self.dropout = dropout
        self.dropout_layer = nn.Dropout(p=prob)
    ...
    def forward(self, x_in):
        ...
        if self.dropout:
            output = self.output(self.dropout_layer(x))
        else:
